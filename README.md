...
- **自動化 per-field accuracy 已有(離線基準)**：對一份 human-validated gold standard 已做
  自動化逐欄位評分(最佳地端模型治療欄位 ~98.5%、CRM 決策 99.5%);該基準/評分程式在研究端
  (llm-bench),未隨本 repo 發布。**production 端本來就沒有 gold standard** —— 那正是
  verify + ensemble QC(gold-free)存在的理由。
